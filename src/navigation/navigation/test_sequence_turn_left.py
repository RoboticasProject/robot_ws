#!/usr/bin/env python3
"""
test_sequence_turn_left.py — Turn right 90° → straight 10 cm → turn right 90°
Run:  python3 src/navigation/navigation/test_sequence_turn_left.py
"""
...
PUL_TURN_D     = 1200    # right wheel drives → 90° left turn
...
# ── phase helpers ──────────────────────────────────────────────────────────────
def turn_left_90():
    """Right wheel forward, left stopped, until right encoder reaches PUL_TURN_D."""
    enc_D.reset_abs()
    _moteur('G', 0)
    _moteur('D', SPEED_TURN)
    while enc_D.get_abs() < PUL_TURN_D:
        time.sleep(0.002)
    _stop()
    print(f"  turn done : {enc_D.get_abs()} pulses (right wheel)")
